...
void testOfficerAssignment(CrimeService& service) {
    ...
    // Test officer capacity more realistically
    cout << "Testing officer O4 capacity (maxLoad=6):" << endl;
    // Try to assign multiple crimes to O4 to test capacity
    for (int i = 0; i < 7; i++) {
        string testCrimeId = "TEST" + to_string(i);
        CrimeReport testCrime{ testCrimeId, "Test", 1, getCurrentEpoch(), "A1", "Capacity test" };
        service.addCrime(testCrime);

        bool assigned = service.assignOfficer(testCrimeId, "O4");
        cout << "  Assigning crime " << testCrimeId << " to O4: "
            << (assigned ? "SUCCESS" : "FAILED (capacity)") << endl;
    }
    cout << endl;
}
...
void testRecentCrimes(CrimeService& service) {
    ...
    vector<string> recent = service.getRecentCrimes(3);
